...
def SumOfSquares(clusters, centroids, type):
    ...
    for i, cluster in enumerate(clusters):
        for point in cluster.getPoints():
            if type == 'E':
                sum += EuclideanDistance(point, centroids[i])
            elif type == 'M':
                sum += ManhattanDistance(point, centroids[i])
...
def DunnIndex(clusters, centroids, type):
    ...
    for i1, c1 in enumerate(centroids):
        for i2, c2 in enumerate(centroids):
            ...
            dist = 0
            if type == 'E':
                dist = EuclideanDistance(c1, c2)
            elif type == 'M':
                dist = ManhattanDistance(c1, c2)
            distClusters.append(dist)

    minDistClusters = 0
    if len(distClusters) != 0:
        minDistClusters = min(distClusters)
